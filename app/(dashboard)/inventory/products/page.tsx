...
import { Plus, Download, Filter, ScanLine } from "lucide-react";

type Product = typeof MOCK_PRODUCTS[0];

const columns: Col<Product>[] = [
  { key:"id", header:"ID", width:"100px", render:r=><span style={{fontFamily:"'DM Mono',monospace",fontSize:11,color:"var(--tx-3)"}}>{r.id}</span> },
  ...
  { key:"quantity", header:"Qty", render:r=><span style={{fontWeight:600,fontFamily:"'DM Mono',monospace"}}>{r.quantity.toLocaleString()}</span> },
...
export default function ProductsPage() {
  ...
  return (
    <div>
      <PageHeader title="Drug Inventory" subtitle={`${MOCK_PRODUCTS.length} products tracked across all warehouses`}
        action={<>
          ...
          <button className="btn-primary btn-sm"><Plus className="w-3.5 h-3.5" />Add Product</button>
        </>}
      />
      {/* Category filter */}
      ...
      <DataTable columns={columns} data={filtered as unknown as Record<string,unknown>[]} searchKeys={["name","gtin","sku","batch"] as never} pageSize={10} />
    </div>
  );
}